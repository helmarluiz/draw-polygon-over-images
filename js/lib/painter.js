...
var painter = function () {
	var self = this;
	container = document.getElementById('canvas-ctt');
	this.imgurl = '';
	this.jsonstr = '';
	this.callback;
	
	this.points = [[]];
	this.colors = ['#128fff', '#06e836'];
	...
	this.setColor = function(){
		var color = $(this).attr('color') || 1;
		console.log('color:', color, self.active, self.colors)
	...
	this.drawSingle = function (points, p) {
		...
		self.ctx.fillStyle = 'rgb(255, 85, 100, 0.3)';
		...
		self.ctx.font = "normal normal 14px Helvetica"; // set font weight, size, etc
		self.ctx.textBaseline = "middle"; // how to align the text vertically
		self.ctx.textAlign = "end"; // how to align the text horizontally
		self.ctx.fillText("test1", 10, 50); // text, x, y
		self.ctx.closePath();
		self.ctx.fill();
		self.ctx.stroke();
	};
			...
			for(var i = 0; i < self.points.length; i++) {
				...
				if (self.points[i].length != 4 || (self.points[i].length == 4 && !(
					...
					for (var j in self.points[i]) {
						...
						/* dont include the lat coordinates, as it's the same as the first coordinates */
						if((parseInt(j) + 1) == self.points[i].length)
						{
							continue;
						}
